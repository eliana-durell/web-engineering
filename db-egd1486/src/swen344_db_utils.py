...
def exec_get_one(sql, args={}): #follows the DRY principle
    conn = connect()
    cur = conn.cursor()
...
# @author Eliana Durell
def exec_returning_all(sql, args={}):
    """
    Fetches all the RETURNING parameters
    """
    conn = connect()
    cur = conn.cursor()
    cur.execute(sql, args)
...
# @author Eliana Durell
def exec_returning_many(sql, args={}):
    """
    Fetches all rows with the RETURNING parameters,
    """
    conn = connect()
    cur = conn.cursor()
    cur.execute(sql, args)
